...
public class blockedInventory {
	...
	public static String inv_name = MessageUtils.format("&c&lBLOCKED PLAYERS");
	
	public blockedInventory(oPlayer player) {
		this.player = player;
	...
	public void organiseFriendList() {
		...
		for(UUID id : player.getBlocklist()) {
	...
	public int generateInventorySize() {
		int i = 18;
		if(!player.getBlocklist().isEmpty()) {
			for(int x = 9; x <= player.getBlocklist().size(); x=x+9) {
				i = i+x;
			}
		} else {
			i = i + 9;
